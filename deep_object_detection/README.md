...
```
4. Append the python path of Caffe to the existing Python path:
```
export PYTHONPATH=/path_to_caffe-fast-rcnn/caffe-fast-rcnn/python:$PYTHONPATH
```
5. Using the *download_faster_rcnn_models.sh* script, download the Caffe model files.
6. Go to the *libpyfaster* directory under the *src* directory of the package and run make. *Hint:* If you have an older GPU such as 6XX series, then in the `setup.py` file, change the `-arch=sm_35` option to `-arch=sm_30` and run make.
